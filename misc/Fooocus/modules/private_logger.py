...
def log(img, metadata, metadata_parser: MetadataParser | None = None, output_format=None) -> str:
    ...
    print(f"Saving image to: {local_temp_filename}")  # Print the path where the image will be saved
    ...
    print(f"Image saved to: {local_temp_filename}")  # Confirm the image has been saved
    if os.path.exists(local_temp_filename):
        print(f"Confirmed: Image file exists at {local_temp_filename}")  # Verify the image file exists
    else:
        print(f"Error: Image file does not exist at {local_temp_filename}")  # Indicate if the image file does not exist
